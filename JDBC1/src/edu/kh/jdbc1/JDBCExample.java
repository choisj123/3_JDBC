...
public class JDBCExample {
	public static void main(String[] args) {
		...
		// SELECT 하는 방법!! (INSERT, UPDATE, DELETE 는 다름)
		// 1단계 : JDBC 객체 참조 변수 선언 (java.sql 패키지의 인터페이스)
		
		Connection conn = null;  //중요!!
		// DB 연결 정보를 담은 객체 ! 
		// --> DBMS 타입, 이름, IP, Port, 계정명, 비밀번호 저장
		// --> DBeaver 계정 접속 방법과 유사함
		// * DB와 Java 사이를 연결해주는 통로(Stream과 유사함)
		
		Statement stmt = null; // java.sql 꼭 확인! (java.beans XXX)
		// Connection을 통해 
		// SQL문을 DB에 전달하여 실행하고 
		// 생성된 결과(ResultSet, 성공한 행의 개수)를 반환(Java에게)하는데 사용되는 객체
		...
		try {
			// 2단계 : 참조 변수에 알맞은 객체 대입 --> try ~ catch
			...
			// --> 생략해도 자동으로 메모리 로드가 진행됨 But, 명시적으로 작성하는걸 권장
			...
			// oracle.jdbc.driver.T4CConnection@59309333
			...
			// 3단계 : SQL을 수행해서 반환 받은 결과(ResultSet)를 한행씩 접근해서 컬럼 값 얻어오기 --> try ~ catch
