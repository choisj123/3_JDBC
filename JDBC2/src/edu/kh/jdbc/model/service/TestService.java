...
// 클래스명을 생략할 수 있게하는 구문
// import static ~~~ .*
import static edu.kh.jdbc.common.JDBCTemplate.*; // 메서드 부를 때 클래스명 안써도 됨!
...
public class TestService {
	...
	private TestDAO dao = new TestDAO(); // TestDAO() 기본생성자
	...
	public int insert(TestVO vo1) throws SQLException {
		...
		// 트랜잭션 제어
		if (result > 0)
			commit(conn); // JDBCTemplate
		else
			rollback(conn); // JDBCTemplate
	...
	public int insert(TestVO vo1, TestVO vo2, TestVO vo3) throws Exception {
		...
		} catch (SQLException e) { // dao 수행 중에 예외가 발생했을 때
			rollback(conn);

			// -> 데이터 삽입 실패된 데이터를 DB에 삽입하지 않음
			// -> DB에는 성공된 데이터만 저장이 된다
			// == DB에 저장된 데이터의 신뢰도가 상승된다
